...
The accel_df contains `r nrow(accel_df)` observations of activity (accelerometer data), and there are `r ncol(accel_df)` variables, which are `r names(accel_df)` that describing the information of data.
...
The trend of total activity counts for each day is dramatically changed over five weeks.